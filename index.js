...
function processLastItem(stringList, callback) {
  /* CODE HERE */
  //result returning last element in the array
...
// reduce callback
// accum -> what we have accumulated so far
 ...
 * should return 1000.
*/
//function takes 3 arguments 2 numbers and a callback
function processProduct(num1, num2, callback) {
 return callback(num1 * 3);
}
...
function processContains(item, list, callback) {
  //checking to see if item is in list useing if statement
  if(list.includes(item)){
    return callback(true) //if item is in the list - callback will be true
  }
  return callback(false)//if item is not in the list - callback will be false
}
...
//function takes 2 arguments an array and a callback function

//using filter method on array - creating a new array 
function processDuplicateFree(list, callback) {

  let newList = [];//create a new array
  for (let i = 0; i < list.length; i++) { 
    //do the duplicate numbers exist in the new array
...
function getFullNames(runners) {
  const runnerNames = [];
  runners.forEach(runners => runnerNames.push(`${runners.last_name}, ${runners.first_name}`))
  return runnerNames;
}
...
function firstNamesAllCaps(runners) {
  let NameCaps = runners.map (runnerName => runnerName.first_name.toUpperCase());
  return NameCaps;
}
...
function getRunnersByTShirtSize(runners, tShirtSize) {
  ...
  //return the array containing the runners with the given tShirtSize

  let runnerTShirt = runners.filter(runner => runner.shirt_size === tShirtSize);
  return runnerTShirt;
}
/**
